...
static PQCQMIMSG ComposeQMUXMsg(UCHAR QMIType, USHORT Type, CUSTOMQMUX customQmuxMsgFunction, void *arg) {
    ...
    if (qmiclientId[pRequest->QMIHdr.QMIType] == 0) {
    ...
    *pPSAttachedState = 0;
    ...
    //0x02    PS_DETACHED ?Detached
        *pPSAttachedState = pServingSystem->RegistrationState;
        if (pServingSystem->RegistrationState == 0x01) //0x01 每 REGISTERED 每 Registered with a network
...
__requestSetupDataCall:
    ...
    if (le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXResult) || le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXError))
    {
        ...
        if (old_auth == profile->auth && profile->user && profile->user[0] && profile->password && profile->password[0]) {
...
#if 0
        if (old_IPType == profile->IPType && QMI_ERR_PIN_LOCKED == le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXError))
        {
            profile->IPType = (profile->IPType == 0x04) ? 0x06 : 0x04;
            free(pResponse);
            goto __requestSetupDataCall;
        }
#endif
