...
static int plc_cmd_init(void)
{
        ...
        if (g_config.uloop_fd_cmd.fd > 0)
        {
                ...
                PLC_DBG_PRINTF(PLC_INFO, "plc-gateway add cmd fd:%d\n", g_config.uloop_fd_cmd.fd );  
...
static void plc_sendto_service(char *topic, char * p_msg,  int msg_len)
{
        ...
        /* init sun_path */
        mqttclient_sockaddr.sun_family = AF_UNIX;
        snprintf(mqttclient_sockaddr.sun_path, sizeof(mqttclient_sockaddr.sun_path), "/var/run/%s.local", "mac_onoffline");

        /* bind socket */
        mqttclient_addr_len = strlen(mqttclient_sockaddr.sun_path) + sizeof(mqttclient_sockaddr.sun_family);

        bytes = sendto(g_config.uloop_fd_client.fd, plc_publish_msg, publish_msg_len, 0, (struct sockaddr*)&mqttclient_sockaddr, mqttclient_addr_len);
...
static void plc_sendmsg_2_mqttclient()
{
        ...
        strcpy(pc_msg, "yunJDWX/jdwx_exec/post/plc");
...
static void plc_sendto_service_test()
{
        ...
        /* init sun_path */
        mqttclient_sockaddr.sun_family = AF_UNIX;
        snprintf(mqttclient_sockaddr.sun_path, sizeof(mqttclient_sockaddr.sun_path), "%s", g_config.srvaddr_name);
...
static void plc_gateway_try_connect_service(void)
{
        ...
        if (g_config.uloop_fd_client.fd <= 0)
        {
                ...
                if (g_config.uloop_fd_client.fd > 0)
                {
                        ...
                        PLC_DBG_PRINTF(PLC_GATEWAY, "[plc-gateway] connect ok, uloop_fd_client fd:%d\n", g_config.uloop_fd_client.fd );  

                }

                PLC_DBG_PRINTF(PLC_GATEWAY, "[plc-gateway] try connect service fd:%d\n", g_config.uloop_fd_client.fd );  

        }
        

        PLC_DBG_PRINTF(PLC_GATEWAY, "[plc-gateway] try connect service\n");  
        return;
}
...
{
	printf("plc-gateway version %s (build date %s)\n", g_config.version, g_config.buildtime);
	printf("Usage: plc-gateway [-d] [-l] [-h] [--conn-num num] [--scan-num num]\n");
	printf("               [test]\n");
	printf("       plc-gateway --help\n\n");
...
int plc_option_proc(int argc, char *argv[])
{
        ...
        for (i=1; i<argc; i++)
        {
                ...
                {
                        printf("[PLC-GATEWAY] Welcome to plc-gateway, Revision:%s (build date:%s)\n"
