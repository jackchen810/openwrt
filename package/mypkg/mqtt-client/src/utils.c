...
// is_https_proxy will return 1 if url is header of "https:"
// 		or return 0 with out it/error
int 
is_https_proxy(char *url) {
	if (url == NULL || strlen(url) < 9) {
		return 0;
	}

	char headerbuf[7] = {0};
	snprintf(headerbuf, sizeof(headerbuf), "%s", url);
	if( STRCMP(headerbuf, ==, "https:")) {
		return 1;
	}
	...
	*state_code = CURL_OK;
	...
	if (curl) {
		...
		*/ 
		...
		if (is_https_proxy(url)) {
			printf("curl: is https!!!!\n");
...
// return 0: failed, 1:is net port
int 
is_net_port(const char *port) {
	if (port == NULL || strlen(port) < 1) {
		return 0;
	}

	char *endptr = NULL;
	unsigned int num = strtoumax(port, &endptr, 10);
 
	if ((endptr && strlen(endptr)) || num == 0) {
		return 0;
	}
